...
// Definição de cores da interface
var (
	corBorda        = tcell.ColorDeepSkyBlue
	corFundo        = tcell.ColorBlack
	corTexto        = tcell.ColorWhite
	corRotulo       = tcell.ColorAqua
...
const tempoLimiteComando = 5 * time.Second

// Executa comando com timeout
func executarComandoComTimeout(nome string, args ...string) ([]byte, error) {
	ctx, cancelar := context.WithTimeout(context.Background(), tempoLimiteComando)
	defer cancelar()

	cmd := exec.CommandContext(ctx, nome, args...)
	saida, err := cmd.Output()
	if ctx.Err() == context.DeadlineExceeded {
		return nil, fmt.Errorf("tempo limite excedido ao executar comando %s", nome)
	}
	return saida, err
}

// Modos de configuração IPv4/IPv6
const (
	ModoIPv6Auto      = "Auto"
	ModoIPv6Manual    = "Manual"
	ModoIPv6Desligado = "Desligado"
	ModoIPv4Auto      = "Auto"
	ModoIPv4Manual    = "Manual"
)

// Valores padrão para configuração IPv4
const (
	IPPadrao         = "192.168.1.100"
	MascaraPadrao    = "24"
	GatewayPadrao    = "192.168.1.1"
	DNS1Padrao       = "8.8.8.8"
	DNS2Padrao       = "8.8.4.4"
)

// Valores padrão para configuração IPv6
const (
	IPv6Padrao       = "2001:db8::1"
	PrefixoIPv6Padrao = "64"
	GatewayIPv6Padrao = "2001:db8::1"
	DNSIPv6Padrao1    = "2001:4860:4860::8888"
...
func ShowNetworkStatus(app *tview.Application) *tview.Flex {
	...
	table.SetBorderColor(corBorda)
	table.SetTitle(" 📊 " + i18n.T("network_status") + " 📊 ")
	table.SetTitleColor(corTitulo)
	table.SetTitleAlign(tview.AlignCenter)
	table.Select(0, 0)
	table.SetSelectable(true, false)
	table.SetFixed(1, 0)

	table.SetBackgroundColor(corFundo)
	...
	if err != nil {
		...
		if devMode {
			...
			buttonsForm.SetBackgroundColor(corFundo)
	...
	for row, conn := range connections {
		...
		rowData := []struct {
			...
			{conn.Device, corTexto},
			{conn.Type, corTexto},
			{conn.State, stateColor},
			{conn.Name, corTexto},
			{conn.IPv4, corTexto},
			{conn.IPv6, corTexto},
			{conn.Gateway, corTexto},
			{conn.DNS, corTexto},
		}
...
func ConfigureNetwork(app *tview.Application) *tview.Flex {
	...
	form.SetBorder(true).
		...
		SetTitleAlign(tview.AlignCenter).
		SetTitleColor(corTitulo).
		SetBorderColor(corBorda).
		SetBackgroundColor(corFundo).
		SetBorderPadding(2, 2, 3, 3)

	// Configurando cores dos campos do formulário
	form.SetFieldBackgroundColor(corFundoCampo)
	form.SetFieldTextColor(corTexto)
	form.SetLabelColor(corRotulo)
	form.SetButtonBackgroundColor(corFundoBotao)
	form.SetButtonTextColor(corTextoBotao)
	...
	mode_options := []string{ModoIPv4Auto, ModoIPv4Manual}
	...
	form.AddInputField(i18n.T("network_ipv4_address"), IPPadrao, 20, nil, nil)
	form.AddInputField(i18n.T("network_ipv4_netmask"), MascaraPadrao, 20, nil, nil)
	form.AddInputField(i18n.T("network_ipv4_gateway"), GatewayPadrao, 20, nil, nil)
	form.AddInputField(i18n.T("network_ipv4_dns1"), DNS1Padrao, 20, nil, nil)
	form.AddInputField(i18n.T("network_ipv4_dns2"), DNS2Padrao, 20, nil, nil)
	...
	ipv6_options := []string{ModoIPv6Auto, ModoIPv6Manual, ModoIPv6Desligado}
	...
	form.AddInputField(i18n.T("network_ipv6_address"), IPv6Padrao, 40, nil, nil)
	form.AddInputField(i18n.T("network_ipv6_prefix"), PrefixoIPv6Padrao, 20, nil, nil)
	form.AddInputField(i18n.T("network_ipv6_gateway"), GatewayIPv6Padrao, 40, nil, nil)
	form.AddInputField(i18n.T("network_ipv6_dns1"), DNSIPv6Padrao1, 40, nil, nil)
	form.AddInputField(i18n.T("network_ipv6_dns2"), DNSIPv6Padrao2, 40, nil, nil)
